...
export const AdminePannelTableComponent = () => {
    ...
    useEffect(() => {
      (async () => {
        try {
          const response = await axios.get("http://localhost:5000/user/findAdmine");
          // handle success
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="-mb-0.5 text-body-4 font-medium text-metal-600">{admineUsers?.name}</p>
                    <span className="text-body-6 font-normal text-metal-500">{admineUsers?.name?.replace(/\d+/g, '')}</span>
                  </div>
                </div>
              </div>
            </div>
